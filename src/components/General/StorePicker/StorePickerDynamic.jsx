...
const StorePickerDynamic = () => {
	...
	const selectedStoreGroups = useStore(state => state.selectedStoreGroups);
	const setStores = useStore(state => state.setStores);
	const setStoresSelected = useStore(state => state.setStoresSelected);
	const setSelectedStoreGroups = useStore(
		state => state.setSelectedStoreGroups,
	);
	...
	const storesSelectedItem = () => {
		if (stores.length === 0) return [];
		const storesNames = [];
		if (storesSelected === null && selectedStoreGroups === null) {
			storesNames.push(stores[0].name);
	...
	const [selectedItem, setSelectedItem] = useState(storesSelectedItem());
	...
	const SaveBtn = () => {
		return (
			...
			>
				<p className='font-baloo text-sm'> Save as Group</p>
			</button>
		);
	};
	...
	const obtainStores = async () => {
		const storesDB = await getStores(user.clients[0].clientId);
		const storeGroupsDB = await getStoreGroups(user.clients[0].clientId);
		setStores(storesDB);
	...
	const handleSelectAllStores = () => {
		if (selectedMultiStores.length === stores.length) {
			setSelectedMultiStores([]);
		} else {
			setSelectedMultiStores(stores.map(store => store.storeId));
		}
	};
	...
	return (
		<div
			...
			} relative w-[212px] h-12 flex items-center px-4 rounded-xl border-2 hover:border-2 hover:border-primary-purple-600 hover:bg-dark-accent-00`}
		>
			<button
					...
					{(stores.length || storeGroups.length) && (
						<div className='h-[42px] flex items-center border-b border-b-neutrals-600 border-dashed'>
							<button
								...
								className={`${
									selectedItem[0] === 'All Stores' && 'bg-primary-purple-600'
						...
						{stores.map((store, index) => (
							<div
								...
								className={`${
									selectedItem[0] === 'All Stores'
										? 'bg-transparent'
										: selectedItem[0] === store.name && 'bg-primary-purple-900'
							...
							<div className='w-full h-[495px] flex flex-col gap-[10px] py-4 px-5 bg-dark-popup-00 rounded-xl'>
								<div className='w-full flex justify-between items-center gap-6 whitespace-nowrap'>
									...
									<button
										...
										onClick={() => setSelectedMultiStores([])}
									>
										Clear All
									</button>
								...
								<div className='h-[42px] px-4 flex items-center gap-2'>
									<input
										...
										checked={selectedMultiStores.length === stores.length}
									/>
									<label
									...
									{filteredStores.map((store, index) => (
										...
										>
											<input
												...
												id={store.name}
												name={store.name}
												checked={selectedMultiStores.includes(store.storeId)}
												onChange={() => handleSelectMultiStores(store.storeId)}
												className='border-[1px] border-neutrals-600 bg-transparent checked:bg-purple-600 checked:border-transparent accent-primary-purple-900 rounded'
											/>
											<label
												htmlFor={store.name}
...
export default StorePickerDynamic;
